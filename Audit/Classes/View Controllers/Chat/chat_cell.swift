...
class chat_cell: UITableViewCell {
    ...
    func setUpLangaugeSetting() {
        ...
        if img_media != nil {
            img_media.transform = CGAffineTransform(scaleX: kAppDelegate.intViewFlipStatus, y: 1)
        }
    ...
    func setSenderChatData(obChat: ChatModel) {
        ...
        if obChat.msgtype == 1 {
            ...
            if kAppDelegate.strLanguageName == LanguageType.Arabic {
                lbl_message.transform = CGAffineTransform(scaleX: kAppDelegate.intViewFlipStatus, y: 1)
                lbl_message.textAlignment = NSTextAlignment.right
            }
        }
    ...
    func setReceiverChatData(obChat: ChatModel) {
        ...
        img_profilepic.backgroundColor = .white
        if obChat.msgtype == MessageType.Text {
            lbl_message.text = obChat.msg
        }   else if obChat.msgtype == MessageType.Image {
